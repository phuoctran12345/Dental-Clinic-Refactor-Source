...
CREATE TABLE [dbo].[Reexamination] (
    [reexam_id]         INT            IDENTITY(1,1) PRIMARY KEY,
    [appointment_id]    INT            NOT NULL,     -- Link đến appointment (có thể là gốc hoặc tái khám)
    ...
    INDEX IX_ReexamSimple_AppointmentId ([appointment_id]),
    INDEX IX_ReexamSimple_Status ([status]),
    INDEX IX_ReexamSimple_Type ([reexam_type])
);
